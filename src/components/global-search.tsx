...
export function GlobalSearch() {
  ...
  return <>{open ? <div className="fixed inset-0 z-40 flex items-start justify-center bg-background/80 p-4 pt-[18vh] backdrop-blur-sm" onClick={() => setOpen(false)}><form className="w-full max-w-lg rounded-xl border border-border bg-card p-3 shadow-xl" onClick={(event) => event.stopPropagation()} onSubmit={submit}><div className="flex items-center gap-2"><MagnifyingGlass className="size-4 text-muted-foreground" /><Input autoFocus className="border-0 bg-transparent shadow-none focus-visible:ring-0" onChange={(event) => setQuery(event.target.value)} placeholder="Buscar leads por nome ou telefone..." value={query} /></div><p className="mt-2 px-2 text-xs text-muted-foreground">A busca atual consulta Leads. Pressione Enter para ver resultados · Esc para fechar</p></form></div> : null}<Button aria-label="Buscar leads (Ctrl K)" className="hidden gap-2 text-muted-foreground md:flex" onClick={() => setOpen(true)} size="sm" variant="outline"><MagnifyingGlass /> Buscar <kbd className="rounded border border-border px-1.5 text-[10px]">Ctrl K</kbd></Button></>;
}
