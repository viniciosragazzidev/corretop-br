...
export function WhatsAppConnectDialog({ initial, triggerLabel = "Conectar WhatsApp" }: { initial: Connection; triggerLabel?: string }) {
  ...
  function updateStatus(status: string) { previousStatus.current = status; setConnection((current) => ({ ...current, status, chatInternoAtivo: status === "ready" ? true : current.chatInternoAtivo })); }
  async function pollStatus() { if (polling.current) return; polling.current = true; try { const result = await getWhatsAppSessionStatus(); if (!result.success || !result.status) return; if (result.status === "ready") { const wasReady = previousStatus.current === "ready"; updateStatus("ready"); if (!wasReady) toast.success("WhatsApp conectado. O chat interno está ativo."); setOpen(false); } else updateStatus(result.status); } finally { polling.current = false; } }
  useEffect(() => { if (!open || !connection.sessionId || connection.status === "ready") return; void pollStatus(); const timer = window.setInterval(() => startTransition(async () => pollStatus()), 1000); return () => window.clearInterval(timer); }, [open, connection.sessionId, connection.status]);
