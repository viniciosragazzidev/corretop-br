...
export function CorreTopFinanceiroSidebar() {
  ...
  return <Sidebar collapsible="icon" variant="sidebar" rail><SidebarHeader><SidebarMenu><SidebarMenuItem><SidebarMenuButton className="h-10" size="lg" render={<Link href="/financeiro" prefetch />}><span className="grid size-7 place-items-center rounded-md bg-emerald-600 text-xs font-bold text-white">$</span><span className="truncate font-semibold tracking-tight">Financeiro</span></SidebarMenuButton></SidebarMenuItem></SidebarMenu><div className="mx-2 rounded-md border border-sidebar-border bg-sidebar-accent/45 px-3 py-2.5 group-data-[collapsible=icon]:hidden"><p className="text-[11px] text-sidebar-foreground/55">Área financeira</p><p className="mt-0.5 truncate text-sm font-medium">{userRole || "Gestão Financeira"}</p></div></SidebarHeader><SidebarContent><NavigationGroup items={overviewItems} label="Visão geral" roleKey={roleKey} groupIndex={0} /><NavigationGroup items={commissionItems} label="Comissões" roleKey={roleKey} groupIndex={1} /><NavigationGroup items={goalsItems} label="Metas" roleKey={roleKey} groupIndex={2} /><NavigationGroup items={reportsItems} label="Relatórios" roleKey={roleKey} groupIndex={3} /><NavigationGroup items={settingsItems} label="Configurações" roleKey={roleKey} groupIndex={4} /></SidebarContent><SidebarFooter><SidebarMenu><SidebarMenuItem><SidebarMenuButton size="lg" render={<button type="button" onClick={handleLogout} />} tooltip={userName}><span className="grid size-7 place-items-center rounded-full bg-sidebar-accent text-xs font-semibold">{initials}</span><span className="grid flex-1 text-left text-sm leading-tight"><span className="truncate font-medium">{userName}</span><span className="truncate text-xs text-sidebar-foreground/55">{userRole}</span></span><SignOut className="ml-auto size-4 shrink-0 text-sidebar-foreground/55 group-data-[collapsible=icon]:hidden" /></SidebarMenuButton></SidebarMenuItem></SidebarMenu></SidebarFooter></Sidebar>;
}
