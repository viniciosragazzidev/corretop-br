...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const rawItems = await db.select({ planId: schema.quoteItems.planId, monthlyPrice: schema.quoteItems.monthlyPrice, recommended: schema.quoteItems.recommended, snapshot: schema.quoteItems.snapshot })
  ...
  const [legacyPlanDetails, globalPlanDetails] = await Promise.all([
    planIds.length > 0
      ? db.select({ id: schema.carrierPlans.id, planName: schema.carrierPlans.name, carrierName: schema.carriers.name })
      ...
      : Promise.resolve([]),
    planIds.length > 0
      ? db.select({ id: schema.globalPlans.id, planName: schema.globalPlans.name, carrierName: schema.globalCarriers.name })
