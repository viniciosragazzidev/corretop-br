...
export default async function PublicQuotePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (!quote) notFound();
  const rawItems = await db.select({ id: schema.quoteItems.id, planId: schema.quoteItems.planId, monthlyPrice: schema.quoteItems.monthlyPrice, recommended: schema.quoteItems.recommended, snapshot: schema.quoteItems.snapshot }).from(schema.quoteItems).where(eq(schema.quoteItems.quoteId, quote.id));
  ...
  const items = rawItems.map((item) => {
    ...
    return { id: item.id, planName: details?.planName ?? (snapshot?.planName as string) ?? "Plano", carrierName: details?.carrierName ?? (snapshot?.carrierName as string) ?? "Operadora", coverage: details?.coverage ?? null, monthlyPrice: item.monthlyPrice, recommended: item.recommended };
  });
