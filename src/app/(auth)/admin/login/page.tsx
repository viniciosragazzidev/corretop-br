...
export default function AdminLoginPage() {
  ...
  async function submit(formData: FormData) { setLoading(true); setError(""); toast.info("Validando acesso administrativo..."); try { const result = await signIn.email({ email: String(formData.get("email")), password: String(formData.get("password")), callbackURL: "/super-admin" }); if (result.error) { setError(result.error.message || "Credenciais inválidas"); toast.error(result.error.message || "Credenciais inválidas"); setLoading(false); return; } toast.success("Acesso administrativo confirmado."); router.push("/super-admin"); } catch (error) { const message = error instanceof Error ? error.message : "Não foi possível concluir o login."; setError(message); toast.error(message); setLoading(false); } }
