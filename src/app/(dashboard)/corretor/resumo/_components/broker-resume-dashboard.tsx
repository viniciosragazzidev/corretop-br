...
function statusLabel(status: string) { return ({ new: "Novo", distributed: "Distribuído", in_contact: "Em contato", quote_sent: "Cotação", negotiation: "Negociação", documentation_pending: "Documentação", under_analysis: "Em análise", converted: "Convertido", lost: "Perdido" } as Record<string, string>)[status] ?? status }
...
export function BrokerResumeDashboard({ embedded = false, data }: { embedded?: boolean; data: BrokerDashboardData }) {
  ...
  const metrics = [
    { label: "Leads na carteira", value: String(data.totals.all), change: `${data.totals.active} ativos`, description: "dados do seu tenant", trend: "up" as const, chart: { total: data.totals.all, label: "leads", segments: [{ name: "ativos", value: data.totals.active, color: "var(--primary)" }, { name: "fechados", value: data.totals.converted, color: "var(--chart-2)" }] } },
    { label: "Novos leads", value: String(data.totals.new), change: "aguardando contato", description: "fila pessoal", trend: "up" as const, chart: { total: data.totals.new, label: "novos", segments: [{ name: "novos", value: data.totals.new, color: "var(--chart-3)" }, { name: "restante", value: Math.max(0, data.totals.all - data.totals.new), color: "var(--muted)" }] } },
    { label: "Em atendimento", value: String(data.totals.inContact), change: "status atual", description: "contatos em andamento", trend: "up" as const, chart: { total: data.totals.inContact, label: "ativos", segments: [{ name: "contato", value: data.totals.inContact, color: "var(--chart-4)" }, { name: "restante", value: Math.max(0, data.totals.all - data.totals.inContact), color: "var(--muted)" }] } },
  ]
  return <DashboardShell embedded={embedded}>
    ...
    <div className="flex flex-1 flex-col gap-5 p-4 lg:gap-6 lg:p-6"><div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div><p className="text-xs font-medium text-primary">DADOS REAIS</p><h2 className="mt-1 text-2xl font-semibold tracking-tight lg:text-3xl">Olá, {data.userName.split(" ")[0]}.</h2><p className="mt-1 text-sm text-muted-foreground">Acompanhe sua carteira pelo módulo de Leads.</p></div><div className="flex flex-wrap items-center gap-2"><AvailabilityToggle initialStatus={data.availabilityStatus} /><Button render={<a href="/leads" />} size="sm"><Plus weight="bold" /> Novo lead</Button></div></div>
      {data.activeLeads.length ? <Card className="border-emerald-300/30 bg-emerald-300/5 shadow-none"><CardContent className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-emerald-300/15 text-emerald-300"><BellRinging size={18} weight="bold" /></span><div><p className="text-sm font-semibold text-emerald-200">Atendimento ativo</p><p className="mt-1 text-sm text-muted-foreground">Você está responsável por {data.activeLeads.length} {data.activeLeads.length === 1 ? "lead" : "leads"}. Mantenha o contato e avance o status.</p><div className="mt-2 flex flex-wrap gap-1.5">{data.activeLeads.slice(0, 3).map((lead) => <Badge className="border-emerald-300/30 bg-emerald-300/10 text-emerald-200" key={lead.id} variant="outline">{lead.name}</Badge>)}{data.activeLeads.length > 3 ? <Badge variant="outline">+{data.activeLeads.length - 3}</Badge> : null}</div></div></div><Button render={<a href="/leads" />} size="sm" variant="outline">Ver meus atendimentos <ArrowRight /></Button></CardContent></Card> : null}
