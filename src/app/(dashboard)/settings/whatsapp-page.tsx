...
export function WhatsAppPage({ initial, returnTo, official }: { initial: Connection; returnTo?: string; official: OfficialSetup }) {
  ...
  return <main className="flex min-h-full flex-col gap-6 bg-background p-4 lg:p-6">
    ...
    <Card className="border-border bg-card shadow-none"><CardHeader><CardTitle>Conexão QR legada</CardTitle><CardDescription>Alternativa temporária durante a migração. Para o canal empresarial da Meta, use “Conectar número oficial” acima.</CardDescription></CardHeader><CardContent className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-sm font-semibold">{ready ? "QR legado conectado" : "QR legado não conectado"}</p><p className="mt-1 text-xs text-muted-foreground">{ready ? "O chat interno legado está pronto para os atendimentos." : "Abra esta alternativa somente se a conexão oficial ainda não estiver disponível."}</p></div><WhatsAppConnectDialog connectedLabel="QR legado conectado" initial={initial} returnTo={returnTo} triggerLabel="Abrir QR legado" /></CardContent></Card>
