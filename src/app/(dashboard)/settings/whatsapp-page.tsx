...
export function WhatsAppPage({ initial, returnTo, official }: { initial: Connection; returnTo?: string; official: OfficialSetup }) {
  ...
  return <main className="flex min-h-full flex-col gap-6 bg-background p-4 lg:p-6">
    ...
    <MetaCloudSetupCard appId={official.appId} branches={official.branches} canConfigure={official.canConfigure} channels={official.channels} companyAccount={official.companyAccount} configId={official.embeddedSignupConfigId} configured={official.configured} enabled={official.enabled} missing={official.missing} />
    <Card className="border-border bg-card shadow-none"><CardHeader><CardTitle>{official.canConfigure ? "Conexão pessoal legada (opcional)" : "Conexão QR legada"}</CardTitle><CardDescription>{official.canConfigure ? "Esta conexão é individual e não altera o status da conta WhatsApp da empresa." : "Alternativa temporária durante a migração. O canal empresarial é administrado pelo Diretor."}</CardDescription></CardHeader><CardContent className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-sm font-semibold">{ready ? "QR legado conectado" : "QR legado não conectado"}</p><p className="mt-1 text-xs text-muted-foreground">{ready ? "O chat interno legado está pronto para os atendimentos." : "Use somente enquanto o canal oficial não estiver disponível."}</p></div><WhatsAppConnectDialog connectedLabel="QR legado conectado" initial={initial} returnTo={returnTo} triggerLabel="Abrir QR legado" /></CardContent></Card>
