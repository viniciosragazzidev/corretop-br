...
import { registerOpenWaWebhook } from "@/lib/integrations/openwa";
...
async function getOwnConnection() {
  ...
  const [connection] = await getDatabase().select().from(schema.whatsappConnections).where(and(eq(schema.whatsappConnections.tenantId, context.tenantId), eq(schema.whatsappConnections.userId, context.userId))).limit(1);
  return { context, db: getDatabase(), connection };
}
...
export async function startWhatsAppConnection() {
  ...
  try {
    if (!connection?.sessionId) {
      ...
      const values = { id: randomUUID(), tenantId: context.tenantId, userId: context.userId, sessionId: session.id, sessionName: session.name ?? null, status: session.status ?? "created", webhookSecret: randomUUID(), updatedAt: new Date() };
    ...
    await db.update(schema.whatsappConnections).set({ status: "initializing", chatInternoAtivo: true, updatedAt: new Date() }).where(eq(schema.whatsappConnections.id, connection.id));
    return { success: true };
...
export async function refreshWhatsAppQr() {
  ...
  try { const response = await fetch(`${baseUrl()}/sessions/${connection.sessionId}/qr`, { headers: headers(), cache: "no-store" }); const data = await response.json() as { qrCode?: string; status?: string }; if (!response.ok) throw new Error("QR ainda não disponível. Aguarde alguns segundos."); await db.update(schema.whatsappConnections).set({ qrCode: data.qrCode ?? null, status: data.status ?? connection.status, connectedAt: data.status === "ready" ? new Date() : connection.connectedAt, chatInternoAtivo: data.status === "ready" ? true : connection.chatInternoAtivo, updatedAt: new Date() }).where(eq(schema.whatsappConnections.id, connection.id)); return { success: true, qrCode: data.qrCode ?? null, status: data.status ?? connection.status }; } catch (error) { return { success: false, error: error instanceof Error ? error.message : "Não foi possível atualizar o QR Code." }; }
}
...
export async function getWhatsAppSessionStatus() {
  ...
  try { const response = await fetch(`${baseUrl()}/sessions/${connection.sessionId}`, { headers: headers(), cache: "no-store" }); if (!response.ok) throw new Error("Não foi possível consultar o status do WhatsApp."); const data = await response.json() as { status?: string; phone?: string }; const status = data.status ?? connection.status; await db.update(schema.whatsappConnections).set({ status, connectedAt: status === "ready" ? connection.connectedAt ?? new Date() : connection.connectedAt, chatInternoAtivo: status === "ready" ? true : connection.chatInternoAtivo, updatedAt: new Date() }).where(eq(schema.whatsappConnections.id, connection.id)); return { success: true, status, phone: data.phone ?? null }; } catch (error) { return { success: false, error: error instanceof Error ? error.message : "Não foi possível consultar o status." }; }
}
