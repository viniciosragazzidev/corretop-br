...
export default async function SettingsPage() {
  ...
  const [tenant, user, membership] = await Promise.all([
    ...
    context.branchId ? db.select({ id: schema.branches.id, name: schema.branches.name, status: schema.branches.status, acceptingLeads: schema.branches.acceptingLeads, autoDistribute: schema.branches.autoDistribute, createdAt: schema.branches.createdAt }).from(schema.branches).where(and(eq(schema.branches.id, context.branchId), eq(schema.branches.tenantId, context.tenantId))).limit(1) : Promise.resolve([]),
  ]);
