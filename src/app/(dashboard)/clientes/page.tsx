...
export default async function CustomersPage() {
  ...
  const clients = await getDatabase().select({ id: schema.clients.id, name: schema.clients.nome, phone: schema.clients.telefone, email: schema.clients.email, convertedAt: schema.clients.convertedAt, brokerName: schema.user.name, branchName: schema.branches.name }).from(schema.clients).leftJoin(schema.user, eq(schema.clients.corretorId, schema.user.id)).leftJoin(schema.branches, eq(schema.clients.branchId, schema.branches.id)).where(and(eq(schema.clients.tenantId, context.tenantId), context.role === "broker" ? eq(schema.clients.corretorId, context.userId) : context.role === "manager" && context.branchId ? eq(schema.clients.branchId, context.branchId) : undefined)).orderBy(desc(schema.clients.convertedAt));
