...
export function RegisterSalePanel({ leadId, documents }: { leadId: string; documents: ConfirmationDocument[] }) {
  ...
  function submit() {
    startTransition(async () => {
      const result = await registerSaleAction({ leadId, policyNumber, coverageStartDate, approvedValue, confirmationDocumentId });
      if (result.error) {
  ...
  return (
    <Card className="border-primary/20 bg-primary/[0.03] shadow-none">
      ...
      <CardContent className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-1.5"><Label htmlFor="policy-number">Número da apólice</Label><Input id="policy-number" value={policyNumber} onChange={(event) => setPolicyNumber(event.target.value)} /></div>
