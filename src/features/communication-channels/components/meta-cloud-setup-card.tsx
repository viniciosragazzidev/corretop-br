...
export function MetaCloudSetupCard({ enabled, configured, missing, appId, configId, canConfigure, branches, channels }: { enabled: boolean; configured: boolean; missing: string[]; appId: string | null; configId: string | null; canConfigure: boolean; branches: { id: string; name: string }[]; channels: Channel[] }) {
  ...
  useEffect(() => {
    ...
    startTransition(async () => {
      try {
        const result = await completeMetaEmbeddedSignupAction({ code, ...metadata, ...(branchId ? { branchId } : {}) });
        toast.success("Canal oficial conectado.", { description: result.displayPhoneNumber ?? "A Meta já pode enviar e receber mensagens neste canal." });
        window.location.reload();
      } catch (error) {
        submitted.current = false;
        setCode(null);
        toast.error(error instanceof Error ? error.message : "Não foi possível concluir a conexão oficial.");
      }
    });
  }, [branchId, code, metadata]);
  ...
  function startSignup() {
    ...
    window.FB.login((response) => {
      ...
      if (!nextCode) { toast.error("A Meta não devolveu o código de autorização. Tente novamente."); return; }
      setCode(nextCode);
    }, { config_id: configId, response_type: "code", override_default_response_type: true, extras: { feature: "whatsapp_embedded_signup" } });
  }

  return <Card className="border-border bg-card shadow-none">
    <CardHeader><CardTitle>WhatsApp oficial da Meta</CardTitle><CardDescription>Canal empresarial conectado pelo Embedded Signup. Tokens ficam cifrados no servidor; nenhum segredo é enviado ao navegador.</CardDescription></CardHeader>
    <CardContent className="space-y-4">
      ...
      {enabled && configured && canConfigure ? <div className="flex flex-wrap items-end gap-3 rounded-lg border border-border bg-muted/20 p-3"><label className="grid gap-1 text-sm"><span>Unidade do canal</span><select className="h-9 rounded-md border border-input bg-background px-2" value={branchId} onChange={(event) => setBranchId(event.target.value)}><option value="">Toda a corretora</option>{branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></label><Button disabled={!sdkReady || isPending} onClick={startSignup} type="button">{isPending ? "Concluindo conexão..." : "Conectar número oficial"}</Button><p className="text-xs text-muted-foreground">A Meta abrirá o cadastro da WABA e do número. Ao concluir, este canal será associado a esta corretora.</p></div> : null}
