...
export async function createQuoteAction(input: z.infer<typeof quoteInput>): Promise<QuoteActionState> {
  ...
  try {
    ...
    const catalogPlanMap = new Map(availableCatalogPlans.map((plan) => [plan.planId, plan]));
    const catalogPlans = parsed.data.planIds
      .filter((id) => !legacyPlans.some((legacyPlan) => legacyPlan.id === id))
      .map((id) => catalogPlanMap.get(id))
      .filter((plan): plan is NonNullable<typeof plan> => plan !== undefined);

    const allValidPlanIds = new Set([...legacyPlans.map((plan) => plan.id), ...catalogPlans.map((plan) => plan.planId)]);
    ...
    const plans = [
      ...legacyPlans.map((plan) => ({ id: plan.id, name: plan.name, carrierName: plan.carrierName, source: "legacy" as const })),
      ...catalogPlans.map((plan) => ({ id: plan.planId, name: plan.planName, carrierName: plan.carrierName, source: plan.source as "global" | "tenant_private" })),
    ];

    const legacyPlanIds = plans.filter((plan) => plan.source === "legacy").map((plan) => plan.id);
    const globalPlanIds = plans.filter((plan) => plan.source === "global").map((plan) => plan.id);
    const [legacyPrices, globalPrices] = await Promise.all([
      ...
      globalPlanIds.length > 0
        ? db.select({ planId: schema.globalPlans.id, ageBand: schema.catalogPriceRows.ageBand, monthlyPrice: schema.catalogPriceRows.monthlyPrice })
            ...
            .where(and(eq(schema.catalogTableVersions.status, "published"), eq(schema.catalogPriceTables.status, "published"), inArray(schema.globalPlans.id, globalPlanIds)))
        : Promise.resolve([]),
    ]);
