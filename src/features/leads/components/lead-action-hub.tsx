...
export function LeadActionHub({ leadId, status, currentOwner, hasQuotes, hasPendingDocuments, nextTask }: LeadActionHubProps) {
  ...
  return <section aria-labelledby="lead-next-action-title" className="rounded-xl border border-primary/20 bg-primary/[0.04] p-4 shadow-none sm:p-5">
    ...
    <nav aria-label="Atalhos do lead" className="mt-3 flex flex-wrap gap-2"><Button className="h-8 px-2.5 text-xs" render={<Link href={"/conversas?leadId=" + leadId} />} size="sm" variant="outline"><ChatCircleText /> Conversar</Button><Button className="h-8 px-2.5 text-xs" render={<Link href={"/tarefas?leadId=" + leadId} />} size="sm" variant="outline"><ListChecks /> Tarefas</Button><Button className="h-8 px-2.5 text-xs" render={<Link href={"/cotacoes?leadId=" + leadId} />} size="sm" variant="outline"><Calculator /> Cotação</Button><Button className="h-8 px-2.5 text-xs" render={<Link href="#documentos" />} size="sm" variant="outline"><FileText /> Documentos{hasPendingDocuments ? " · pendentes" : ""}</Button></nav>
