    ...
    <DialogPopup className="h-[80vh] w-[70vw] max-w-none overflow-hidden p-0 max-lg:w-[86vw] max-sm:h-[88vh] max-sm:w-[calc(100vw-1rem)]" overlayClassName="bg-black/65 backdrop-blur-sm">
      <div className="flex h-full min-h-0 flex-col bg-background">
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          {!active && canSend ? <div className="m-6 flex flex-wrap items-center justify-between gap-4 rounded-xl border border-warning/30 bg-accent/10 p-5"><div><p className="text-sm font-medium text-warning-foreground">WhatsApp não configurado</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Você precisa conectar seu WhatsApp para enviar mensagens. A configuração é rápida — leia o QR Code com seu celular.</p></div><Button className="shrink-0" render={<Link href={`/settings/whatsapp?returnTo=${encodeURIComponent(pathname)}`} />} size="sm" variant="outline"><WhatsappLogo /> Conectar agora</Button></div> : <>
            <div className="border-b border-warning/20 bg-accent/10 px-6 py-3"><div className="flex items-start gap-2"><ShieldWarning className="mt-0.5 shrink-0 text-warning" size={16} /><p className="text-xs leading-4 text-warning-foreground">Apenas mensagens relacionadas a este lead aparecem aqui.</p></div></div>
