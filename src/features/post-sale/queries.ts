...
export async function getLeadBeneficiaries(leadId: string) {
  ...
  return getDatabase().select({ id: schema.leadBeneficiaries.id, name: schema.leadBeneficiaries.name, birthDate: schema.leadBeneficiaries.birthDate, relationship: schema.leadBeneficiaries.relationship, isHolder: schema.leadBeneficiaries.isHolder }).from(schema.leadBeneficiaries).where(and(eq(schema.leadBeneficiaries.leadId, leadId), eq(schema.leadBeneficiaries.tenantId, context.tenantId), context.role === "broker" ? eq(schema.leads.corretorId, context.userId) : context.role === "manager" ? eq(schema.leads.branchId, context.branchId ?? "") : undefined)).innerJoin(schema.leads, eq(schema.leadBeneficiaries.leadId, schema.leads.id)).orderBy(schema.leadBeneficiaries.isHolder, asc(schema.leadBeneficiaries.name));
}
