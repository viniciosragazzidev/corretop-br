...
- **Fonte da credencial:** `source` aceita `site_pixel`, `meta_ads` e `landing_page`, mas o intake sincrono grava `distributionOrigin: "landing-page"`. Pixel e Meta Ads nao possuem contrato/rota de entrada distintos para testar.
...
4. Contrato de cada provedor futuro (Meta/Pixel): assinatura, mapeamento de payload, retry e origem persistida.
...
| P0 | Unificar o contrato publico de webhook e preservar a fonte configurada pela credencial; so entao expor adaptadores para Pixel/Meta. | Sim, pois altera contrato e origem |
